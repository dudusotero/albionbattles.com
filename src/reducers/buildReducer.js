  ...
  async ({ item, itemType }, { rejectWithValue }) => {
    try {
      const { data } = await axios.get(
        `http://localhost:5001/items/${item.uniqueName}`
      );
      console.log(data);
...
export const saveBuild = createAsyncThunk(
  "build/saveBuild",
  async (_, { rejectWithValue, getState }) => {
    const build = getState().build;
    try {
      axios
        .post(`http://localhost:5001/build`, { build })
        .then((response) => {
...
export const { removeItem, setSpells, selectSpell, setDescription } =
  slice.actions;
