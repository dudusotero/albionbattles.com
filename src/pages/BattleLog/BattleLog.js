...
const formatName = (alliances) => {
  ...
  if (alliances.length > 4) {
    return `${alliances.slice(0, 3).join(", ")} and ${
      alliances.slice(4, alliances.length).length
    } more`;
  }
...
const formatDescription = ({ alliances, time, kills, players }) => {
  const string = `Battle: ${formatName(
    alliances
  )} at ${time} - ${players} players and ${kills} kills`;
  return string;
};
...
const BattleLog = (props) => {
  ...
  let alliances =
    filter !== "" && battle.alliances.alliances.length > 0
      ? battle.alliances.alliances.filter((a) =>
          a.name.toLowerCase().includes(filter.toLowerCase())
        )
      : battle.alliances.alliances;
  let guilds =
    filter !== "" && battle.guilds.guilds.length > 0
      ? battle.guilds.guilds.filter((a) =>
          a.alliance.toLowerCase().includes(filter.toLowerCase())
        )
      : battle.guilds.guilds;
  let players =
    filter !== "" && battle.players.players.length > 0
      ? battle.players.players.filter((a) =>
          a.allianceName.toLowerCase().includes(filter.toLowerCase())
        )
