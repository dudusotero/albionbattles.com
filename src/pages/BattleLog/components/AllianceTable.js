...
const AllianceTable = ({ alliances: a }) => {
  ...
  const maxKills =
    alliances.length > 0
      ? alliances.sort((a, b) => b.kills - a.kills)[0].kills
      : null;
  const maxKillFame =
    alliances.length > 0
      ? alliances.sort((a, b) => b.killFame - a.killFame)[0].killFame
      : null;
      ...
      >
        <FlexboxGrid>
          ...
          <FlexboxGrid.Item componentClass={Col} md={3} smHidden xsHidden>
            <p style={{ textAlign: "right", paddingRight: 15 }}>
              {intToString(alliance.totalDamage)}
            </p>
          </FlexboxGrid.Item>
          <FlexboxGrid.Item componentClass={Col} md={3} smHidden xsHidden>
  ...
  return (
    ...
    >
      ...
      <List hover>{visible.map(renderAllianceStats)}</List>
      <div
        style={{
          marginLeft: "auto",
          position: "absolute",
