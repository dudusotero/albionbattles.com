...
const tooltip = (
  <Tooltip>
    Albion API only provides the top 4 damage/healing contributors to a kill so
    the damage/healing tracker may be inaccurate.
  </Tooltip>
);
...
const PlayerTable = ({ players: p }) => {
  ...
  const renderPlayerStats = (player) => {
    const item =
      player.equipment && player.equipment.mainhand
      ...
      <List.Item key={player.id}>
        <FlexboxGrid>
          ...
          <FlexboxGrid.Item componentClass={Col} md={2} xs={4} sm={4}>
            <p style={{ textAlign: "right", paddingRight: 15 }}>
              {intToString(player.killFame)}
            </p>
          </FlexboxGrid.Item>
          <FlexboxGrid.Item componentClass={Col} md={2} xs={4} sm={4}>
            <p style={{ textAlign: "right", paddingRight: 15 }}>
              {intToString(player.totalDamage)}
            </p>
          </FlexboxGrid.Item>
          <FlexboxGrid.Item componentClass={Col} md={2} xs={3} sm={3}>
            <p style={{ textAlign: "right", paddingRight: 15 }}>
              {intToString(player.totalHealing)}
            </p>
          </FlexboxGrid.Item>
          <FlexboxGrid.Item componentClass={Col} md={2} xs={4} sm={4}>
  ...
  return (
    ...
    >
      ...
      <List hover>{visible.map(renderPlayerStats)}</List>
      <div
        style={{
          marginLeft: "auto",
          position: "absolute",
