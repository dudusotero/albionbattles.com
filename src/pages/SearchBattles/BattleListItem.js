...
const formatName = (alliances, toShow) => {
  ...
  if (alliances.length > toShow) {
    return `${alliances.slice(0, toShow).join(", ")} + ${
      alliances.slice(toShow, alliances.length).length
    }`;
  }
...
const RenderBattle = ({ battle }) => {
  ...
  const mobileDateDisplay = moment(battle.startTime)
    .add(7, "hours")
    .format("MM-DD");
    ...
    return (
      <List.Item>
        <FlexboxGrid
          align="middle"
          style={{
            color: battle.totalKills < 20 ? "#999999" : "#EEEEEE",
            paddingLeft: "0.5rem",
          }}
        >
          {multi && (
            <Checkbox
              ...
              checked={multiIds.includes(battle.id)}
            ></Checkbox>
          )}
          <FlexboxGrid.Item
            componentClass={Col}
          ...
          <FlexboxGrid.Item componentClass={Col} md={3} xs={3}>
            <p
              style={{
                textAlign: "right",
                color: battle.totalFame > 999999 ? "#29e09d" : null,
              }}
            >
              {intToString(battle.totalFame)}
            </p>
          </FlexboxGrid.Item>
          <FlexboxGrid.Item componentClass={Col} md={3} xs={3}>
            <p
              style={{
                textAlign: "right",
                color: battle.players.list.length > 19 ? "#29cee0" : null,
              }}
            >
          ...
          <FlexboxGrid.Item componentClass={Col} md={2} xs={3}>
            <p
              style={{
                textAlign: "right",
                color: battle.totalKills > 19 ? "rgba(224,41,131,1)" : null,
              }}
            >
              {battle.totalKills}
            </p>
          ...
          <FlexboxGrid.Item componentClass={Col} md={3} xsHidden smHidden>
            {multi ? (
              ...
              >
                <p style={{ textAlign: "right", color: "rgb(133, 97, 197)" }}>
                  {battle.id}
                </p>
              </Link>
            ) : (
  ...
  return (
    <Link style={{ textDecoration: "none" }} to={`/battles/${battle.id}`}>
      {renderFields()}
    </Link>
  );
};
